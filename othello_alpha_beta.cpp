...
int min_value(char maxplayer,char minplayer,int depth_count,char board_min[8][8],int parent_i,int parent_j,int parent_cost)
 {
	...
	if(depth_count==depth-1)
	{
		...
		int legal_move[64];
		int mov_i_new=-1,mov_count=-1,mov_ij_new=-1;
		found_move=false;
		max_count=0;
		for(int j=0;j<8;j++)
    	{
    	...
    	if(board_min[j][k]=='*')
    	             {
						...
						opp_player=maxplayer;
						  ...
						  if(found_move==true)
    	             	 {
                				...
                				{
								output1<<"minif"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<"Infinity"<<endl;
								}
								else
								{
									output1<<"minif"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<parent_cost_minif<<endl;
								}
								char print_j= 97+mov_j;
                				output1<<"minf"<<print_j<<mov_i+1<<","<<depth_count+1<<","<<mov_i_new<<endl;    									
						  ...
						  if(found_move==true)
    	             	 {
    	             	 	pass=false;
    	             	 	 return_val=cost;
                				...
                				if(parent_cost==999)
                				{
								output1<<"minelse"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<"Infinity"<<endl;
								//output1<<"root,0,"<<"Infinity"<<endl;
								}
								else
								{
									output1<<"minelse"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<parent_cost<<endl;
								}
                     			return_val=max_value(maxplayer,minplayer,depth_count+1,board_final_final,mov_i,mov_j,(-max_return));
								output1<<"minelse"<<print_val<<mov_i+1<<","<<depth_count+1<<","<<return_val<<endl;
    	...
    	if(board_max[j][k]=='*')
    	             {
						...
						opp_player=minplayer;
						  ...
						  if(found_move==true)
    	             	 {
    	             	 	    mov_i_new=find_weight(minplayer,maxplayer);
                				...
                				if(max_return==-999)
                				{	
								output1<<"maxifparent"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<"-Infinity"<<endl;
								}
								else
								{
									output1<<"maxif"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<abs(max_return)<<endl;
								}
								char print_j= 97+mov_j;
                				output1<<"maxif"<<print_j<<mov_i+1<<","<<depth_count+1<<","<<mov_i_new<<endl;
						...
						opp_player=minplayer;
						  ...
						  if(found_move==true)
    	             	 {
    	             	 	pass=false;
								...
								output1<<"depth"<<print_val<<mov_i+1<<","<<depth_count+1<<","<<return_val_depth<<endl;
                     			output1<<"depth"<<"root,0,"<<return_val_depth<<endl;
						...
						opp_player=minplayer;
						  ...
						  if(found_move==true)
    	             	 {
    	             	 	pass=false;
								...
								output1<<"maxelse"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<"-Infinity"<<endl;
								//output1<<"root,0,"<<"Infinity"<<endl;
								}
								else
								{
									output1<<"maxelse"<<print_parent_j<<parent_i+1<<","<<depth_count<<","<<abs(parent_cost_else)<<endl;
								}cout<<"max_return_else_pass"<<max_return_else_pass;
                     			return_val_else=min_value(maxplayer,minplayer,depth_start+1,board_final_max,mov_i,mov_j,(-max_return_else_pass));
